...
La aplicación espera PostgreSQL, Kafka y Neo4j. La configuración se obtiene de variables de entorno; consulta `application.yml` y el `.env.example` raíz. Al ejecutarla fuera de Docker, los valores por defecto usan los puertos publicados por Compose: PostgreSQL `4532`, Kafka `4992` y Neo4j Bolt `4787`; la API local escucha en `8989`.
