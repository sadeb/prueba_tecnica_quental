...
**Estado**: Propuesta · **Requisito**: [spec/05](../spec/05-api-propia.md) puntos 2–3; bonus B1 ([spec/08](../spec/08-bonus.md)) · **Fecha**: 2026-09-12
...
- `/api/admin/sync`: protegido con token de cualquier usuario autenticado (no hay roles en el alcance). Anotar en README que en un entorno real requeriría rol admin.
- Registro: `username` único (409 si existe), `password` mínimo 8 caracteres, almacenada con BCrypt.
- Sin refresh token, sin revocación, sin roles: fuera de alcance; se dice explícitamente en README.
