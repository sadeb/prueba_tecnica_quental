...
| `POST /api/admin/sync` | sí, rol `ADMIN` ([ADR-012](../decisions/ADR-012-administrador-sistema.md)) | 202 `{runId, startedAt}` | 401, 403 sin rol, 409 sync en curso |
| `GET /api/admin/sync/{runId}` | sí, rol `ADMIN` | 200 estado y contadores | 401, 403, 404 |
...
- Listados sin auth: los datos sincronizados son públicos; solo `/api/users/me/**` (cualquier usuario) y `/api/admin/**` (rol `ADMIN`) requieren token; el resto es `permitAll` para que una ruta no mapeada responda 404 y no 401 ([formato-error](formato-error.md)).
