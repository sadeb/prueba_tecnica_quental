...
## Pasos
1. Migración `V3__users.sql`: `users(id, username UNIQUE, password_hash, created_at)`.
2. `User` entidad + `UserRepository`.
3. `TokenService`: `issue(user)` y `parse(token)` con `javax.crypto.Mac`, Base64 URL sin padding, claims `sub`, `uid`, `iat`, `exp`; secreto y TTL desde `auth.token.*`. Errores → `InvalidTokenException`.
...
6. `SecurityConfig` definitiva: stateless, CSRF off, CORS para el origen del frontend, públicas/protegidas según ADR-005, `AuthenticationEntryPoint` que devuelve `ApiError` 401.
