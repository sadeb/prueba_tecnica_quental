...
## Objetivo
Esquema relacional versionado con Liquibase ([ADR-011](../decisions/ADR-011-liquibase-migraciones.md)), entidades JPA y servicios de upsert idempotente por `external_id`.
...
## Pasos
1. Changelog maestro `db/changelog/db.changelog-master.yaml` (YAML) que incluye ficheros formatted SQL en `changes/`. `001-init.sql`: `locations(id, external_id UNIQUE, name, type, dimension, placeholder, created_at, updated_at)`, `episodes(id, external_id UNIQUE, name, air_date, code, placeholder, …)`, `characters(id, external_id UNIQUE, name, status, species, type, gender, image_url, origin_id FK NULL, location_id FK NULL, placeholder, …)`, `character_episodes(character_id, episode_id, PK compuesta)`. Índices en `characters(name)`, `(status)`, `(species)`, `(gender)`.
...
## Hecho cuando
Liquibase aplica `001-init` al arrancar (`databasechangelog` con una fila por changeset); tests del paso en verde (humano).
...
## Commit propuesto
`feat(db): add liquibase schema and idempotent jpa upserts keyed by external id`
