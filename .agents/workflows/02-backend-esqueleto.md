...
## Pasos
1. `backend/pom.xml`: parent `spring-boot-starter-parent` 2.7.18, `java.version` 11, starters `web`, `validation`, `data-jpa`, `data-neo4j`, `kafka`, `security`, `actuator`, `test`, `kafka-test`, driver `postgresql`, `flyway-core`, `springdoc-openapi-ui` 1.7.0, `h2` (test). Maven wrapper.
...
3. `application.yml`: datasource, neo4j, kafka, `sync.*`, `external-api.*`, `auth.token.*` leyendo variables de entorno con valores por defecto para `localhost`. `application-test.yml` con H2 y Kafka embebido.
...
6. `backend/Dockerfile` multi-stage (`eclipse-temurin:11-jdk` build → `11-jre` runtime).
7. Descomentar `backend` en compose ([01](01-infraestructura-docker.md)).
...
## Ejecuta y pega
```bash
cd backend && ./mvnw -q -DskipTests package && echo BUILD_OK
```
